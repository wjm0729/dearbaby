...
/**
 *
 * CompilerContextImpl, implementation of CompilerContext. CompilerContext and
 * hence CompilerContextImpl objects are private to a LanguageConnectionContext.
 *
 */
public class CompilerContextImpl extends ContextImpl implements CompilerContext {
	...
	public Parser getParser() {
		// parser.parseStatement("").accept(v)
	...
	/**
	 * Get the number of subquerys in the current statement from this
	 * CompilerContext.
	 *
	...
	/**
	 * Sets which kind of query fragments are NOT allowed. Basically, these are
	 * fragments which return unstable results. CHECK CONSTRAINTS and CREATE
	 * PUBLICATION want to forbid certain kinds of fragments.
	 *
	 * @param reliability
	 *            bitmask of types of query fragments to be forbidden see the
	 *            reliability bitmasks in CompilerContext.java
	 *
	 */
	...
	/**
	 * Get the compilation schema descriptor for this compilation context. Will
	 * be null if no default schema lookups have occured. Ie. the statement is
	...
	public void popCompilationSchema() {
		SchemaDescriptor sd = defaultSchemaStack.remove(defaultSchemaStack
				.size() - 1);
		setCompilationSchema(sd);
	}
	...
	/**
	 * Sets the current privilege type context. Column and table nodes do not
	 * know how they are being used. Higher level nodes in the query tree do not
	 * know what is being referenced. Keeping the context allows the two to come
	 * together.
	 *
	 * @param privType
	...
	public void addRequiredColumnPriv(ColumnDescriptor column) {
		if (requiredColumnPrivileges == null // Using old style authorization
				|| currPrivType == Authorizer.NULL_PRIV
				|| currPrivType == Authorizer.DELETE_PRIV // Table privilege
															// only
				|| currPrivType == Authorizer.INSERT_PRIV // Table privilege
															// only
				|| currPrivType == Authorizer.TRIGGER_PRIV // Table privilege
															// only
				|| currPrivType == Authorizer.EXECUTE_PRIV || column == null) {
			return;
		}

		/*
		 * Note that to look up the privileges for this column, we need to know
		 * what table the column is in. However, not all ColumnDescriptor
		 * objects are associated with a table object. Sometimes a
		 * ColumnDescriptor describes a column but doesn't specify the table. An
		 * example of this occurs in the set-clause of the UPDATE statement in
		 * SQL, where we may have a ColumnDescriptor which describes the
		 * expression that is being used in the UPDATE statement to provide the
		 * new value that will be computed by the UPDATE. In such a case, there
		 * is no column privilege to be added, so we just take an early return.
		 * DERBY-1583 has more details.
		 */
		...
		StatementTablePermission key = new StatementTablePermission(tableUUID,
				currPrivType);
		StatementColumnPermission tableColumnPrivileges = requiredColumnPrivileges
				.get(key);
		if (tableColumnPrivileges == null) {
			tableColumnPrivileges = new StatementColumnPermission(tableUUID,
					currPrivType, new FormatableBitSet(td.getNumberOfColumns()));
	...
	public void addRequiredRoutinePriv(AliasDescriptor routine) {
		...
		if (routine.getSchemaUUID().toString()
				.equals(SchemaDescriptor.SYSFUN_SCHEMA_UUID))
			return;

		if (requiredRoutinePrivileges.get(routine.getUUID()) == null)
			requiredRoutinePrivileges.put(routine.getUUID(),
	 ...
	 */
	public void addRequiredSchemaPriv(String schemaName, String aid,
			int privType) {
		if (requiredSchemaPrivileges == null || schemaName == null)
			return;

		StatementSchemaPermission key = new StatementSchemaPermission(
				schemaName, aid, privType);
	...
	public void addRequiredRolePriv(String roleName, int privType) {
		...
		StatementRolePermission key = new StatementRolePermission(roleName,
				privType);
	...
	public List<StatementPermission> getRequiredPermissionsList() {
		...
		ArrayList<StatementPermission> list = new ArrayList<StatementPermission>(
				size);
		if (requiredRoutinePrivileges != null) {
			for (Iterator<UUID> itr = requiredRoutinePrivileges.keySet()
		...
		if (requiredUsagePrivileges != null) {
			for (Iterator<UUID> itr = requiredUsagePrivileges.keySet()
					.iterator(); itr.hasNext();) {
				UUID objectID = itr.next();

				list.add(new StatementGenericPermission(objectID,
						requiredUsagePrivileges.get(objectID),
						PermDescriptor.USAGE_PRIV));
			}
		}
		if (requiredTablePrivileges != null) {
			for (Iterator<StatementTablePermission> itr = requiredTablePrivileges
					.values().iterator(); itr.hasNext();) {
				list.add(itr.next());
			}
		}
		if (requiredSchemaPrivileges != null) {
			for (Iterator<StatementSchemaPermission> itr = requiredSchemaPrivileges
					.values().iterator(); itr.hasNext();) {
				list.add(itr.next());
			}
		}
		if (requiredColumnPrivileges != null) {
			for (Iterator<StatementColumnPermission> itr = requiredColumnPrivileges
					.values().iterator(); itr.hasNext();) {
				list.add(itr.next());
			}
		}
		if (requiredRolePrivileges != null) {
			for (Iterator<StatementRolePermission> itr = requiredRolePrivileges
					.values().iterator(); itr.hasNext();) {
				list.add(itr.next());
			}
		}
	...
	public void addReferencedSequence(SequenceDescriptor sd) {
		if (referencedSequences == null) {
			referencedSequences = new HashMap<UUID, SequenceDescriptor>();
		}
	...
	public void addPrivilegeFilter(VisitableFilter vf) {
		if (privilegeCheckFilters == null) {
			privilegeCheckFilters = new ArrayList<VisitableFilter>();
		}
	...
	public void beginScope(String scopeName) {
		if (namedScopes == null) {
			namedScopes = new HashMap<String, int[]>();
		}
	...
	public void endScope(String scopeName) {
		...
		if (scopeDepth[SCOPE_CELL] <= 0) {
			namedScopes.remove(scopeName);
		}
	}
