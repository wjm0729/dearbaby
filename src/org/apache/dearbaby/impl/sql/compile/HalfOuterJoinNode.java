...
class HalfOuterJoinNode extends JoinNode {
	 ...
	 *            Whether or not this node represents a user specified right
	 *            outer join
	 ...
	 *                Thrown on error
	 */
	HalfOuterJoinNode(ResultSetNode leftResult, ResultSetNode rightResult,
			ValueNode onClause, ResultColumnList usingClause,
			boolean rightOuterJoin, Properties tableProperties,
			ContextManager cm) throws StandardException {
		super(leftResult, rightResult, onClause, usingClause, null,
				tableProperties, null, cm);
		this.rightOuterJoin = rightOuterJoin;

		/*
		 * We can only flatten an outer join using the null intolerant predicate
		 * xform. In that case, we will return an InnerJoin.
		 */
	...
	public boolean pushOptPredicate(OptimizablePredicate optimizablePredicate)
		...
		/*
		 * We should never push the predicate to joinPredicates as in JoinNode.
		 * joinPredicates should only be predicates relating the two joining
		 * tables. In the case of half join, it is biased. If the general
		 * predicate (not join predicate) contains refernce to right result set,
		 * and if doesn't qualify, we shouldn't return the row for the result to
		 * be correct, but half join will fill right side NULL and return the
		 * row. So we can only push predicate to the left, as we do in
		 * "pushExpression". bug 5055
		 */
		FromTable leftFromTable = (FromTable) leftResultSet;
		if (leftFromTable.getReferencedTableMap().contains(
	...
	public void genQuery0() {
		FromTable leftFromTable = (FromTable) leftResultSet;
		qm.addNode(leftFromTable.correlationName,
				leftFromTable.origTableName.tableName, this);
		FromTable rightFromTable = (FromTable) rightResultSet;
		qm.addNode(rightFromTable.correlationName,
				rightFromTable.origTableName.tableName, this);
	...
	void pushExpressions(PredicateList outerPredicateList)
		...
		/*
		 * We only try to push single table predicates to the left. Pushing them
		 * to the right would give incorrect semantics. We use the logic for
		 * pushing down single table predicates here.
		 */
		 ...
		 * conditions only get pushed down 1 level. We use the optimizer's logic
		 * for pushing down join clause here.
		 */
	 ...
	 * <ul>
	 * <li>determines if this part of the query tree is a compound OJ of the
	 * shape required for reordering and if so,</li>
	 ...
	 * <ul>
	 * <li>Only left outer joins are considered, i.e. both OJs in diagram above
	 * must be LOJ.</li>
	 * <li>Top left side must be a base table (t1 above). The bottow right side
	 * (t3 above) may be another OJ, so reordering can happen recursively.</li>
	 * </ul>
	 *
	 * @param numTables
	 *            number of tables involved (needed to right size the bitmaps)
	...
	boolean LOJ_reorderable(int numTables) throws StandardException {
		...
		// Redundantly normalize the ON predicate (it will also be called in
		// preprocess()).
		...
		if (logicalLeftResultSet instanceof HalfOuterJoinNode) {
			anyChange = ((HalfOuterJoinNode) logicalLeftResultSet)
					.LOJ_reorderable(numTables) || anyChange;
		} else if (!(logicalLeftResultSet instanceof FromBaseTable)) {// left
																		// operand
																		// must
																		// be
																		// either
		...
		if (logicalRightResultSet instanceof HalfOuterJoinNode) {
			anyChange = ((HalfOuterJoinNode) logicalRightResultSet)
					.LOJ_reorderable(numTables) || anyChange;
		} else if (!(logicalRightResultSet instanceof FromBaseTable)) {// right
																		// operand
																		// must
																		// be
																		// either
		...
		if (rightOuterJoin
				|| (logicalRightResultSet instanceof HalfOuterJoinNode && ((HalfOuterJoinNode) logicalRightResultSet).rightOuterJoin)) {
		...
		RPReferencedTableMap = logicalLeftResultSet
				.LOJgetReferencedTables(numTables);
		NPReferencedTableMap = logicalRightResultSet
				.LOJgetReferencedTables(numTables);
		...
		if (logicalRightResultSet instanceof HalfOuterJoinNode) {
			...
			JBitSet nestedChildOJRPRefTableMap = ((HalfOuterJoinNode) logicalRightResultSet)
					.LOJgetRPReferencedTables(numTables);

			// Checks that top has p(t1,t2)
			if (!isNullRejecting(joinClause, RPReferencedTableMap,
			...
			JBitSet nestedChildOJNPRefTableMap = ((HalfOuterJoinNode) logicalRightResultSet)
					.LOJgetNPReferencedTables(numTables);

			// Checks that right child has p(t2,t3)
			if (isNullRejecting(
					((HalfOuterJoinNode) logicalRightResultSet).joinClause,
					nestedChildOJRPRefTableMap, nestedChildOJNPRefTableMap)) {
				...
				if (super.subqueryList.size() != 0
						|| ((JoinNode) logicalRightResultSet).subqueryList
								.size() != 0
						|| super.joinPredicates.size() != 0
						|| ((JoinNode) logicalRightResultSet).joinPredicates
								.size() != 0
				...
				FromList localFromList = new FromList(getOptimizerFactory()
	 ...
	 * 43-74: <quote> "The set of attributes referenced by a predicate p is
	 * called the schema of p, and denoted sch(p). As a notational convention,
	 * we annotate predicates to reflect their schema. If sch(p) includes
	 * attributes of both Ri, Rj and only those relations, we can write the
	 * predicate as pRiRj. </quote>
	 *
	 * If a null-valued column is compared in a predicate that contains no OR
	 * connectives, the predicate evaluates to undefined, and the tuple is
	 * rejected. The relops satisfy this criterion.
	 * <p/>
	 ...
	 * </pre>
	 *
	 * At least one of the relops should reference both {@code leftTableMap} and
	 * {@code rightTableMap}, so that we know that sch(p) includes attributes of
	 * both Ri, Rj. I.e.
	 *
	 * <p/>
	 * {@code X} should be a table in {@code leftTableMap}, and {@code Y} should
	 * be a table in {@code rightTableMap}.
	 * <p/>
	 * <b>or</b> {@code X} should be a table in {@code rightTableMap}, and
	 * {@code Y} should be a table in {@code leftTableMap}.
	 *
	 * @param joinClause
	 *            The join clause (i.e. predicate) we want to check
	 * @param leftTableMap
	 *            a bit map representing the tables expected for the predicate
	 *            (logical left)
	 * @param rightTableMap
	 *            a bit map representing the tables expected for the predicate
	 *            (logical right)
	 * @return true if the {@code joinClause} has at least one relop that
	 *         references both {@code leftTableMap} and {@code rightTableMap}
	 * @throws StandardException
	 *             standard exception policy
	 */

	private boolean isNullRejecting(ValueNode joinClause, JBitSet leftTableMap,
		...
		while (vn != null) {
			...
			if (vn instanceof BinaryRelationalOperatorNode) {
				...
				if (leftCol instanceof ColumnReference) {
					if (leftTableMap.get(((ColumnReference) leftCol)
							.getTableNumber())) {

						leftFound = true;

					} else if (rightTableMap.get(((ColumnReference) leftCol)
							.getTableNumber())) {
				...
				if (rightCol instanceof ColumnReference) {
					if (leftTableMap.get(((ColumnReference) rightCol)
							.getTableNumber())) {
						leftFound = true;

					} else if (rightTableMap.get(((ColumnReference) rightCol)
							.getTableNumber())) {
						rightFound = true;
					} else {
	...
	FromTable transformOuterJoins(ValueNode predicateTree, int numTables)
		...
		if (predicateTree == null) {
			/*
			 * We can't transform this node, so tell both sides of the outer
			 * join that they can't get flattened into outer query block.
			 */
		...
		/*
		 * Walk predicates looking for a null intolerant predicate on the inner
		 * table.
		 */
		...
		while (vn instanceof AndNode) {
			...
			if (left instanceof RelationalOperator) {
				...
				/*
				 * Do not consider method calls, conditionals, field references,
				 * etc.
				 */
				...
				/*
				 * If the predicate is a null intolerant predicate on the right
				 * side then we can flatten to an inner join. We do the xform
				 * here, flattening will happen later.
				 */
				for (int bit = 0; bit < numTables; bit++) {
					if (refMap.get(bit) && innerMap.get(bit)) {
						// OJ -> IJ
						JoinNode ij = new JoinNode(leftResultSet,
								rightResultSet, joinClause, null,
								getResultColumns(), null, null,
								getContextManager());
		...
		/*
		 * We can't transform this node, so tell both sides of the outer join
		 * that they can't get flattened into outer query block.
		 */
		leftResultSet.notFlattenableJoin();
		rightResultSet.notFlattenableJoin();
	...
	/**
	 * Generate and add any arguments specifict to outer joins. Generate the
	 * methods (and add them as parameters) for returning an empty row from 1 or
	 * more sides of an outer join, if required. Pass whether or not this was
	 * originally a right outer join.
	 *
	 * @param acb
	...
	/**
	 * Return true if right outer join or false if left outer join Used to set
	 * Nullability correctly in JoinNode
	 */
	...
	/**
	 * If this is a right outer join node with USING/NATURAL clause, then check
	 * if the passed ResultColumn is a join column. If yes, then ResultColumn
	 * should be marked such. DERBY-4631
	 */
	@Override
