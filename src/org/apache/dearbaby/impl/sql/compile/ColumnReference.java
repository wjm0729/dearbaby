...
/**
 * A ColumnReference represents a column in the query tree. The parser generates
 * a ColumnReference for each column reference. A column reference could be a
 * column in a base table, a column in a view (which could expand into a complex
 * expression), or a column in a subquery in the FROM clause.
 *
 */

public class ColumnReference extends ValueNode {
	// For associating columns with the SOURCE and TARGET tables of MERGE
	// statements.
	...
	/*
	 * * This is the user-specified table name. It will be null if the* user
	 * specifies a column without a table name. Leave it null even* when the
	 * column is bound as it is only used in binding.
	 */
	public TableName _qualifiedTableName;
	 ...
	 * {@code FromTable}, whereas {@code source} will be bound to the RC in the
	 * result column list of the join node. See also the comment at the end of
	...
	/*
	 * Whether or not this column reference been scoped for the sake of
	 * predicate pushdown.
	 */
	private boolean scoped;

	/*
	 * List of saved remap data if this ColumnReference is scoped and has been
	 * remapped multiple times.
	 */
	private java.util.ArrayList<RemapInfo> remaps;

	/**
	 * Columns mentioned by MERGE statements need to be associated the SOURCE or
	 * TARGET table
	 */
	private int _mergeTableID = MERGE_UNKNOWN;

	/**
	 * Constructor. This one is called by the parser where we could be dealing
	 * with delimited identifiers.
	 *
	 * @param columnName
	 *            The name of the column being referenced
	 * @param tableName
	 *            The qualification for the column
	 * @param tokBeginOffset
	 *            begin position of token for the column name identifier from
	 *            parser.
	 * @param tokEndOffset
	 *            end position of token for the column name identifier from
	 *            parser.
	 * @param cm
	 *            The context manager
	 */
	ColumnReference(String columnName, TableName tableName, int tokBeginOffset,
			int tokEndOffset, ContextManager cm) {
		super(cm);
	...
	public String toString() {
		if (SanityManager.DEBUG) {
			return "columnName: "
					...
					+ "\n"
					+ "tableName: "
					+ ((_qualifiedTableName != null) ? _qualifiedTableName
							.toString() : "null") + "\n" + "nestingLevel: "
	...
	boolean getCorrelated() {
		if (SanityManager.DEBUG) {
			SanityManager.ASSERT(nestingLevel != -1, "nestingLevel on "
					+ _columnName + " is not expected to be -1");
			SanityManager.ASSERT(sourceLevel != -1, "sourceLevel on "
					+ _columnName + " is not expected to be -1");
		}
	...
	/**
	 * Set the nesting level for this CR. (The nesting level at which the CR
	 * appears.)
	 *
	 * @param nestingLevel
	...
	/**
	 * Set the source level for this CR. (The nesting level of the source of the
	 * CR.)
	 *
	 * @param sourceLevel
	...
	/**
	 * Mark this node as being generated to replace an aggregate. (Useful for
	 * replacing aggregates in the HAVING clause with column references to the
	 * matching aggregate in the user's SELECT.
	 */
	...
	/**
	 * Determine whether or not this node was generated to replace an aggregate
	 * in the user's SELECT.
	 *
	 * @return boolean Whether or not this node was generated to replace an
	 *         aggregate in the user's SELECT.
	 */
	...
	/**
	 * Determine whether or not this node was generated to replace a window
	 * function call in the user's SELECT.
	 *
	 * @return boolean Whether or not this node was generated to replace a
	 *         window function call in the user's SELECT.
	 */
	...
	ValueNode getClone() throws StandardException {
		ColumnReference newCR = new ColumnReference(_columnName,
				_qualifiedTableName, getContextManager());
	...
	void copyFields(ColumnReference oldCR) throws StandardException {
		...
		replacesWindowFunctionCall = oldCR
	 ...
	 * figuring out what the return type is for this expression.
	 *
	 * NOTE: We must explicitly check for a null FromList here, column reference
	 * without a FROM list, as the grammar allows the following: insert into t1
	 * values(c1)
	 *
	 * @param fromList
	...
	ValueNode bindExpression(FromList fromList, SubqueryList subqueryList,
		...
		if (SanityManager.DEBUG) {
			SanityManager.ASSERT(fromList != null,
					"fromList is expected to be non-null");
		}

		if (fromList.size() == 0) {
			throw StandardException.newException(
					SQLState.LANG_ILLEGAL_COLUMN_REFERENCE, _columnName);
		}
	...
	/**
	 * Get the column name for purposes of error messages or debugging. This
	 * returns the column name as used in the SQL statement. Thus if it was
	 * qualified with a table, alias name that will be included.
	 *
	...
	/**
	 * Get the user-supplied table name of this column. This will be null if the
	 * user did not supply a name (for example, select a from t). The method
	 * will return B for this example, select b.a from t as b The method will
	 * return T for this example, select t.a from t
	 *
	 * @return The user-supplied name of this column. Null if no user- supplied
	 *         name.
	 */
	@Override
	String getTableName() {
	...
	/**
	 * Get the name of the underlying(base) table this column comes from, if
	 * any. Following example queries will all return T select a from t select
	 * b.a from t as b select t.a from t
	 *
	 * @return The name of the base table that this column comes from. Null if
	...
	/**
	 * Get the name of the schema for the Column's base table, if any. Following
	 * example queries will all return APP (assuming user is in schema APP)
	 * select t.a from t select b.a from t as b select app.t.a from t
	 *
	 * @return The name of the schema for Column's base table. If the column is
	 *         not in a schema (i.e. is a derived column), it returns NULL.
	 */
	...
	/**
	 * Is the column wirtable by the cursor or not. (ie, is it in the list of
	 * FOR UPDATE columns list)
	 *
	 * @return TRUE, if the column is a base column of a table and is writable
	 *         by cursor.
	 */
	@Override
	...
	/**
	 * Set the column number for this ColumnReference. This is used when scoping
	 ...
	 * This step ensures that the top level of the expression is a chain of
	 * AndNodes.
	 *
	...
	ValueNode putAndsOnTop() throws StandardException {
		...
		equalsNode = new BinaryRelationalOperatorNode(
				BinaryRelationalOperatorNode.K_EQUALS, this, trueNode, false,
				getContextManager());
	 ...
	 * could contain subqueries and method calls.
	 *
	 * Also, don't allow a predicate to be pushed down if it contains a
	 * ColumnReference that replaces an aggregate. This can happen if the
	 * aggregate is in the HAVING clause. In this case, we would be pushing the
	 * predicate into the SelectNode that evaluates the aggregate, which doesn't
	 * make sense, since the having clause is supposed to be applied to the
	 * result of the SelectNode. This also goes for column references that
	 * replaces a window function.
	 *
	 *
	 ...
	 * @param simplePredsOnly
	 *            Whether or not to consider method calls, field references and
	 *            conditional nodes when building bit map
	 *
	 * @return boolean Whether or not source.expression is a ColumnReference or
	 *         a VirtualColumnNode or a ConstantNode.
	 */
	@Override
	boolean categorize(JBitSet referencedTabs, boolean simplePredsOnly) {
		...
		return (!replacesAggregate)
				...
				&& ((source.getExpression() instanceof ColumnReference)
						|| (source.getExpression() instanceof VirtualColumnNode) || (source
							.getExpression() instanceof ConstantNode));
	}

	/**
	 * Remap all of the ColumnReferences in this expression tree to point to the
	 * ResultColumn that is 1 level under their current source ResultColumn.
	...
	void remapColumnReferences() {
		...
		/*
		 * Scoped column references are a special case: they can be remapped
		 * several times (once for every ProjectRestrictNode through which the
		 * scoped ColumnReference is pushed before reaching its target result
		 * set) and will be un-remapped several times, as well (as the scoped
		 * predicate is "pulled" back up the query tree to it's original
		 * location). So we have to keep track of the "orig" info for every
		 * remap operation, not just for the most recent one.
		 */
		if (scoped && (_origSource != null)) {
			if (remaps == null)
				remaps = new java.util.ArrayList<RemapInfo>();
			remaps.add(new RemapInfo(columnNumber, tableNumber, _columnName,
					source));
		} else {
			_origSource = source;
		...
		columnNumber = source.getExpression() instanceof VirtualColumnNode ? source
				.getVirtualColumnId() : source.getColumnPosition();
		...
		if (source.getExpression() instanceof ColumnReference) {
			...
			if (SanityManager.DEBUG) {
				// if dummy cr generated to replace aggregate, it may not have
				// table number
				// because underneath can be more than 1 table.
				if (tableNumber == -1 && !cr.getGeneratedToReplaceAggregate()) {
					SanityManager
							.THROWASSERT("tableNumber not expected to be -1, origName = "
									+ origName);
				}
			}
		}
	...
	ResultColumn getSourceResultColumn() {
		/*
		 * RESOLVE - If expression is a ColumnReference, then we are hitting the
		 * top of a query block (derived table or view.) In order to be able to
		 * push the expression down into the next query block, it looks like we
		 * should reset the contents of the current ColumnReference to be the
		 * same as expression. (This probably only means names and tableNumber.)
		 * We would then "rebind" the top level predicate somewhere up the call
		 * stack and see if we could push the predicate through.
		 */
	...
	ValueNode remapColumnReferencesToExpressions() throws StandardException {
		...
		/*
		 * Find the last redundant RC in the chain. We want to clone its
		 * expression.
		 */
		...
		if (SanityManager.DEBUG) {
			if (sourceRC == null) {
				SanityManager
						.THROWASSERT("sourceRC is expected to be non-null for "
								+ _columnName);
			}

			if (!sourceRC.isRedundant()) {
				SanityManager
						.THROWASSERT("sourceRC is expected to be redundant for "
								+ _columnName);
			}
		}

		/*
		 * If last expression is a VCN, then we can't clone it. Instead, we just
		 * reset our source to point to the source of the VCN, those chopping
		 * out the layers. Otherwise, we return a clone of the underlying
		 * expression.
		 */
		if (sourceRC.getExpression() instanceof VirtualColumnNode) {
			VirtualColumnNode vcn = (VirtualColumnNode) (sourceRC
					.getExpression());
			...
			if (rsn instanceof FromTable) {
				...
				/*
				 * It's not enough to just set the table number. Depending on
				 * the original query specified and on whether or not subquery
				 * flattening has occurred, it's possible that the expression to
				 * which we're remapping has a different RCL ordering than the
				 * one to which we were mapped before we got here. In that case
				 * we also need to update the columnNumber to point to the
				 * correct column in "ft". See DERBY-2526 for details. See
				 * DERBY-3023 and DERBY-4679 for further improvement details.
				 */
				...
				ResultColumn ftRC = rcl.getResultColumn(
						...
						columnNumberBeforeFlattening, _columnName);
				...
				/*
				 * Use the virtual column id if the ResultColumn's expression is
				 * a virtual column (DERBY-3023).
				 */
				columnNumber = (ftRC.getExpression() instanceof VirtualColumnNode) ? ftRC
						.getVirtualColumnId() : ftRC.getColumnPosition();
			} else {
				if (SanityManager.DEBUG) {
					SanityManager
							.THROWASSERT("rsn expected to be a FromTable, but is a "
									+ rsn.getClass().getName());
				}
			}
	...
	/**
	 * ColumnReference's are to the current row in the system. This lets us
	 * generate a faster get that simply returns the column from the current
	 * row, rather than getting the value out and returning that, only to have
	 * the caller (in the situations needed) stuffing it back into a new column
	 * holder object. We will assume the general generate() path is for getting
	 * the value out, and use generateColumn() when we want to keep the column
	 * wrapped.
	 *
	 * @exception StandardException
	 *                Thrown on error
	...
	void generateExpression(ExpressionClassBuilder acb, MethodBuilder mb)
		 ...
		 * Most redundant nodes will be flattened out by this point, but* in at
		 * least one case (elimination of redundant ProjectRestricts* during
		 * generation) we don't do this.
		 */
		if (source.isRedundant()) {
		...
		if (SanityManager.DEBUG) {
			if (sourceResultSetNumber < 0) {
				SanityManager
						.THROWASSERT("sourceResultSetNumber expected to be >= 0 for "
								+ getTableName() + "." + getColumnName());
			}
		}

		/*
		 * The ColumnReference is from an immediately underlying ResultSet. The
		 * Row for that ResultSet is Activation.row[sourceResultSetNumber],
		 * where sourceResultSetNumber is the resultSetNumber for that
		 * ResultSet.
		 * 
		 * The generated java is the expression: (<interface>)
		 * this.row[sourceResultSetNumber].getColumn(#columnId);
		 * 
		 * where <interface> is the appropriate Datatype protocol interface for
	...
	/**
	 * Get the user-supplied schema name of this column. This will be null if
	 * the user did not supply a name (for example, select t.a from t). Another
	 * example for null return value (for example, select b.a from t as b). But
	 * for following query select app.t.a from t, this will return APP Code
	 * generation of aggregate functions relies on this method
	 *
	...
	/**
	 * Return whether or not the source of this ColumnReference is itself a
	 * ColumnReference.
	 *
	 * @return Whether or not the source of this ColumnReference is itself a
	 *         ColumnReference.
	...
	/**
	 * The type of a ColumnReference is the type of its source unless the source
	 * is null then it is the type that has been set on this node.
	 */
	@Override
	DataTypeDescriptor getTypeServices() {
		...
		;
		DataTypeDescriptor d = new DataTypeDescriptor(TypeId.BIGINT_ID, 0, 0,
				false, 0);
	...
	/**
	 * Find the source result set for this ColumnReference and return it. Also,
	 * when the source result set is found, return the position (within the
	 * source result set's RCL) of the column referenced by this
	 * ColumnReference. The position is returned vai the colNum parameter.
	 *
	 * @param colNum
	 *            Place to store the position of the column to which this
	 *            ColumnReference points (position is w.r.t the source result
	 *            set).
	 * @return The source result set for this ColumnReference; null if there is
	 *         no source result set.
	 */
	protected ResultSetNode getSourceResultSet(int[] colNum)
			throws StandardException {
		if (source == null) {
			/*
			 * this can happen if column reference is pointing to a column that
			 * is not from a base table. For example, if we have a VALUES clause
			 * like
			 * 
			 * (values (1, 2), (3, 4)) V1 (i, j)
			 * 
			 * and then a column reference to VI.i, the column reference won't
			 * have a source.
			 */
			return null;
		}
		...
		/*
		 * We have to make sure we enter this loop if rc is redundant, so that
		 * we can navigate down to the actual source result set (DERBY-1777). If
		 * rc *is* redundant, then rcExpr is not guaranteed to be a
		 * ColumnReference, so we have to check for that case inside the loop.
		 */
		while ((rcExpr != null)
			 ...
			 * ResultColumn that in turn points to the source expression. This
			 * can happen in cases where "rc" points to a subquery that has been
			 * flattened into the query above it (flattening of subqueries
			 * occurs during preprocessing). In that case we want to skip over
			 * the redundant rc and find the ResultColumn that actually holds
			...
			while (rc.isRedundant()) {
				...
				if (rcExpr instanceof VirtualColumnNode)
					...
					/*
					 * If rc isn't pointing to a VirtualColumnNode nor to a
					 * ColumnReference, then it's not pointing to a result set.
					 * It could, for example, be pointing to a constant node or
					 * to the result of an aggregate or function. Break out of
					 * both loops and return null since there is no source
					 * result set.
					 */
					break;
				}
	...
	boolean isEquivalent(ValueNode o) throws StandardException {
		...
		return (tableNumber == other.tableNumber && _columnName.equals(other
				.getColumnName()));
	}

	/**
	 * Mark this column reference as "scoped", which means that it was created
	 * (as a clone of another ColumnReference) to serve as the left or right
	 * operand of a scoped predicate.
	 */
	...
	void setMergeTableID(int mergeTableID) {
		// Changing the association of a column means we are confused. Shouldn't
		// happen.
		if ((_mergeTableID != MERGE_UNKNOWN) && (_mergeTableID != mergeTableID)) {
			if (SanityManager.DEBUG) {
				SanityManager.ASSERT((_mergeTableID == mergeTableID),
						"MERGE statement can't re-associate column "
								+ getSQLColumnName() + " from "
								+ prettyPrintMergeTableID(_mergeTableID)
								+ " to "
								+ prettyPrintMergeTableID(mergeTableID));
			}
		}
	...
	/**
	 * Helper class to keep track of remap data when a ColumnReference is
	 * remapped multiple times. This allows the CR to be UN- remapped multiple
	 * times, as well.
	 */
