...
public abstract class ResultSetNode extends QueryTreeNode {
	...
	public ResultColumnList resultColumns;
	...
	public String toString() {
		if (SanityManager.DEBUG) {
			return "resultSetNumber: "
					...
					+ "\n"
					+ "referencedTableMap: "
					+ (referencedTableMap != null ? referencedTableMap
							.toString() : "null") + "\n"
	...
	/**
	 * Assign the next resultSetNumber to the resultSetNumber in this
	 * ResultSetNode. Expected to be done during generate().
	 *
	...
	/**
	 * Bind the non VTI tables in this ResultSetNode. This includes getting
	 * their descriptors from the data dictionary and numbering them.
	 *
	...
	/**
	 * Set the type of each parameter in the result column list for this table
	 * constructor.
	 *
	 * @param typeColumns
	 *            The ResultColumnList containing the desired result types.
	 *
	 * @exception StandardException
	 *                Thrown on error
	 */
	...
	/**
	 * Expand "*" into a ResultColumnList with all of the columns in the table's
	 * result list.
	 *
	 * @param allTableName
	 ...
	 *         {@code allTableName} is non-null and doesn't match a table name
	 *         in this result set
	 *
	 * @exception StandardException
	 *                Thrown on error
	...
	ResultColumnList getAllResultColumns(TableName allTableName)
		...
		if (SanityManager.DEBUG)
			SanityManager
					.THROWASSERT("getAllResultColumns() not expected to be called for "
	...
	ResultColumn getMatchingColumn(ColumnReference columnReference)
		...
		if (SanityManager.DEBUG)
			SanityManager
					.THROWASSERT("getMatchingColumn() not expected to be called for "
							+ this);
		return null;
	}

	/**
	 * Set the result column for the subquery to a boolean true, Useful for
	 * transformations such as changing: where exists (select ... from ...) to:
	 ...
	 * 
	 * @param onlyConvertAlls
	 *            Boolean, whether or not to just convert *'s
	 * @return ResultSetNode whose resultColumn was transformed; defaults to
	 *         "this" here, but can be overridden by subclasses.
	 *
	 * @exception StandardException
	 *                Thrown on error
	 */
	ResultSetNode setResultToBooleanTrueNode(boolean onlyConvertAlls)
		...
		BooleanConstantNode booleanNode = new BooleanConstantNode(true,
				getContextManager());
	...
	/**
	 * Get the FromList. Create and return an empty FromList. (Subclasses which
	 * actuall have FromLists will override this.) This is useful because there
	 * is a FromList parameter to bindExpressions() which is used as the common
	 * FromList to bind against, allowing us to support correlation columns
	 * under unions in subqueries.
	 *
	 * @return FromList
	 * @exception StandardException
	 *                Thrown on error
	...
	void bindResultColumns(TableDescriptor targetTableDescriptor,
			...
			throws StandardException {
		/*
		 * For insert select, we need to expand any *'s in the select before
		 * binding the result columns
		 */
		if (this instanceof SelectNode) {
			resultColumns
					.expandAllsAndNameColumns(((SelectNode) this).fromList);
		}

		/*
		 * If specified, copy the result column names down to the source's
		 * target list.
		 */
	...
	/**
	 * Bind untyped nulls to the types in the given ResultColumnList. This is
	 * used for binding the nulls in row constructors and table constructors. In
	 * all other cases (as of the time of this writing), we do nothing.
	 *
	 * @param rcl
	...
	ResultSetNode preprocess(int numTables, GroupByList gbl, FromList fromList)
		...
		if (SanityManager.DEBUG)
			SanityManager
					.THROWASSERT("preprocess() not expected to be called for "
							+ getClass().toString());
		return null;
	}
	...
	ResultSetNode ensurePredicateList(int numTables) throws StandardException {
		if (SanityManager.DEBUG)
			SanityManager
					.THROWASSERT("ensurePredicateList() not expected to be called for "
							+ getClass().toString());
		return null;
	}
	...
	ResultSetNode addNewPredicate(Predicate predicate) throws StandardException {
		if (SanityManager.DEBUG)
			SanityManager
					.THROWASSERT("addNewPredicate() not expected to be called for "
							+ getClass().toString());
		return null;
	}
	 ...
	 * It contains no top level subqueries. (RESOLVE - we can relax this) o It
	 * does not contain a group by or having clause o It does not contain
	 * aggregates.
	 *
	 * @param fromList
	 *            The outer from list
	...
	boolean flattenableInFromSubquery(FromList fromList) {
		if (SanityManager.DEBUG)
			SanityManager
					.THROWASSERT("flattenableInFromSubquery() not expected to be called for "
							+ getClass().toString());
		return false;
	}

	/**
	 * Get a parent ProjectRestrictNode above us. This is useful when we need to
	 * preserve the user specified column order when reordering the columns in
	 * the distinct when we combine an order by with a distinct.
	 *
	...
	ResultSetNode genProjectRestrictForReordering() throws StandardException {
		...
		return new ProjectRestrictNode(this, prRCList, null, /* Restriction */
	...
	/*
	 * * Check whether the column lengths and types of the result columns* match
	 * the expressions under those columns. This is useful for* INSERT and
	 * UPDATE statements. For SELECT statements this method* should always
	 * return true. There is no need to call this for a* DELETE statement.**
	 * @return true means all the columns match their expressions,* false means
	 * at least one column does not match its* expression
	 */
	...
	/**
	 * Get the referencedTableMap for this ResultSetNode. Must be public in
	...
	/**
	 * Check for (and reject) XML values directly under the ResultColumns. This
	 * is done for SELECT/VALUES statements. We reject values in this case
	 * because JDBC does not define an XML type/binding and thus there's no
	 * standard way to pass such a type back to a JDBC application.
	 *
	 * @exception StandardException
	...
	/**
	 * Rename generated result column names as '1', '2' etc... These will be the
	 * result column names seen by JDBC clients.
	 */
	...
	/**
	 * This method is overridden to allow a resultset node to know if it is the
	 * one controlling the statement -- i.e., it is the outermost result set
	...
	/**
	 * This ResultSet is the source for an Insert. The target RCL is in a
	 * different order and/or a superset of this RCL. In most cases we will add
	 * a ProjectRestrictNode on top of the source with an RCL that matches the
	 * target RCL. NOTE - The new or enhanced RCL will be fully bound.
	 *
	 * @param target
	 *            the target node for the insert
	 * @param inOrder
	 *            are source cols in same order as target cols?
	 * @param colMap
	 *            int array representation of correspondence between RCLs -
	 *            colmap[i] = -1 -> missing in current RCL colmap[i] = j ->
	 *            targetRCL(i) <-> thisRCL(j+1)
	 * @return a node that replaces this node and whose RCL matches the target
	 *         RCL. May return this node if no changes to the RCL are needed, or
	 *         if the RCL is modified in-place.
	 *
	 * @exception StandardException
	 *                Thrown on error
	 */
	ResultSetNode enhanceRCLForInsert(InsertNode target, boolean inOrder,
			int[] colMap) throws StandardException {
		if (!inOrder
	...
	/**
	 * Generate an RCL that can replace the original RCL of this node to match
	 * the RCL of the target for the insert.
	 *
	 * @param target
	 *            the target node for the insert
	 * @param colMap
	...
	ResultColumnList getRCLForInsert(InsertNode target, int[] colMap)
		...
		ResultColumnList newResultCols = new ResultColumnList(
				getContextManager());

		/*
		 * Create a massaged version of the source RCL. (Much simpler to build
		 * new list and then assign to source, rather than massage the source
		 * list in place.)
		 */
		...
		for (int index = 0; index < numTargetColumns; index++) {
			...
			if (colMap[index] != -1) {
				...
				newResultColumn = resultColumns
						.getResultColumn(colMap[index] + 1);
			} else {
				newResultColumn = genNewRCForInsert(
						target.targetTableDescriptor, target.targetVTI,
						index + 1, target.getDataDictionary());
			}
	...
	/**
	 * Generate the RC/expression for an unspecified column in an insert. Use
	 * the default if one exists.
	 *
	 * @param targetTD
	 *            Target TableDescriptor if the target is not a VTI, null if a
	 *            VTI.
	 * @param targetVTI
	...
	private ResultColumn genNewRCForInsert(TableDescriptor targetTD,
			FromVTI targetVTI, int columnNumber, DataDictionary dataDictionary)
			throws StandardException {
		ResultColumn newResultColumn;
		...
		if (targetVTI != null) {
			newResultColumn = targetVTI.getResultColumns().getResultColumn(
					columnNumber);
			...
			ColumnDescriptor colDesc = targetTD
					.getColumnDescriptor(columnNumber);
			DataTypeDescriptor colType = colDesc.getType();

			// Check for defaults
			DefaultInfoImpl defaultInfo = (DefaultInfoImpl) colDesc
					.getDefaultInfo();
			...
			if (defaultInfo != null && !colDesc.isAutoincrement()) {
				...
				/*
				 * if (defaultInfo.getDefaultValue() != null) { } else
				 */
				{
					if (colDesc.hasGenerationClause()) {
						// later on we will revisit the generated columns and
						// bind
						// their generation clauses
						newResultColumn = createGeneratedColumn(targetTD,
								colDesc);
					} else {
						...
						defaultTree = defaultTree
								.bindExpression(getFromList(),
										(SubqueryList) null,
										(List<AggregateNode>) null);
						newResultColumn = new ResultColumn(
								defaultTree.getTypeServices(), defaultTree,
								getContextManager());
					}

					DefaultDescriptor defaultDescriptor = colDesc
							.getDefaultDescriptor(dataDictionary);
					if (SanityManager.DEBUG) {
				...
				newResultColumn = new ResultColumn(colType,
	 ...
	 */
	private ResultSetNode generateProjectRestrictForInsert(InsertNode target,
			int[] colMap) throws StandardException {
		// our newResultCols are put into the bound form straight away.
		ResultColumnList newResultCols = new ResultColumnList(
				getContextManager());
		...
		for (int index = 0; index < numTargetColumns; index++) {
			...
			if (colMap[index] != -1) {
				...
				if (dtd == null) {
					ColumnDescriptor cd = target.targetTableDescriptor
							.getColumnDescriptor(index + 1);
					dtd = cd.getType();
				}
				...
				newResultColumn = new ResultColumn(dtd, newColumnReference,
						getContextManager());
			} else {
				newResultColumn = genNewRCForInsert(
						target.targetTableDescriptor, target.targetVTI,
		...
		/*
		 * The generated ProjectRestrictNode now has the ResultColumnList in the
		 * order that the InsertNode expects. NOTE: This code here is an
		 * exception to several "rules": o This is the only ProjectRestrictNode
		 * that is currently generated outside of preprocess(). o The UnionNode
		 * is the only node which is not at the top of the query tree which has
		 * ColumnReferences under its ResultColumnList prior to expression push
		 * down.
		 */
		return new ProjectRestrictNode(this, newResultCols, null, null, null,
	 ...
	 */
	private ResultColumn createGeneratedColumn(TableDescriptor targetTD,
			ColumnDescriptor colDesc) throws StandardException {
		ValueNode dummy = new UntypedNullConstantNode(getContextManager());
		ResultColumn newResultColumn = new ResultColumn(colDesc.getType(),
	...
	public ValueNode parseDefault(String defaultText) throws StandardException {
		...
		/*
		 * We're all set up to parse. We have to build a compilable SQL
		 * statement before we can parse - So, we goober up a VALUES
		 * defaultText.
		 */
		...
		// Since this is always nested inside another SQL statement, so topLevel
		// flag
		// should be false
		...
		if (SanityManager.DEBUG) {
			if (!(qt instanceof CursorNode)) {
				SanityManager
						.THROWASSERT("qt expected to be instanceof CursorNode, not "
								+ qt.getClass().getName());
			}
			CursorNode cn = (CursorNode) qt;
			if (!(cn.getResultSetNode() instanceof RowResultSetNode)) {
				SanityManager
						.THROWASSERT("cn.getResultSetNode() expected to be instanceof RowResultSetNode, not "
	...
	/**
	 * Make a ResultDescription for use in a ResultSet. This is useful when
	 * generating/executing a NormalizeResultSet, since it can appear anywhere
	 * in the tree.
	 *
	...
	/**
	 * Determine if this result set is updatable or not, for a cursor (i.e., is
	 * it a cursor-updatable select). This returns false and we expect
	 * selectnode to refine it for further checking.
	 *
	 * @exception StandardException
	 *                Thrown on error
	 */
	boolean isUpdatableCursor(DataDictionary dd) throws StandardException {
		if (SanityManager.DEBUG)
			SanityManager.DEBUG("DumpUpdateCheck",
					"cursor is not a select result set");
		return false;
	}

	/**
	 * return the target table of an updatable cursor result set. since this is
	 * not updatable, just return null.
	 */
	...
	/**
	 * Mark this ResultSetNode as the target table of an updatable cursor. Most
	 * types of ResultSetNode can't be target tables.
	 * 
	...
	/**
	 * Put a ProjectRestrictNode on top of this ResultSetNode. ColumnReferences
	 * must continue to point to the same ResultColumn, so that ResultColumn
	 * must percolate up to the new PRN. However, that ResultColumn will point
	 * to a new expression, a VirtualColumnNode, which points to the FromTable
	 * and the ResultColumn that is the source for the ColumnReference. (The new
	 * PRN will have the original of the ResultColumnList and the ResultColumns
	 * from that list. The FromTable will get shallow copies of the
	 * ResultColumnList and its ResultColumns. ResultColumn.expression will
	 * remain at the FromTable, with the PRN getting a new VirtualColumnNode for
	 * each ResultColumn.expression.)
	 *
	...
	/**
	 * The optimizer's decision on the access path for a result set may require
	 * the generation of extra result sets. For example, if it chooses an index
	 * for a FromBaseTable, we need an IndexToBaseRowNode above the
	 * FromBaseTable (and the FromBaseTable has to change its column list to
	 * match the index.
	 *
	 * This method in the parent class does not generate any extra result sets.
	 * It may be overridden in child classes.
	 *
	 * @return A ResultSetNode tree modified to do any extra processing for the
	 *         chosen access path
	 *
	 * @exception StandardException
	 *                Thrown on error
	 ...
	 * row, at most. This is important for join nodes where we can save the
	 * extra next on the right side if we know that it will return at most 1
	 * row.
	 *
	 * @return Whether or not the underlying ResultSet tree will return a single
	 *         row.
	 ...
	 *         getOptimizer() method above, then return it; otherwise, return
	 *         null.
	 */
	 ...
	 *
	 * @return Whether or not to materialize this ResultSet tree. would return
	 *         valid results.
	 *
	 * @exception StandardException
	 *                Thrown on error
	...
	FromTable getFromTableByName(String name, String schemaName,
		...
		if (SanityManager.DEBUG) {
			SanityManager
					.THROWASSERT("getFromTableByName() not expected to be called for "
							+ getClass().getName());
		}
		return null;
	}
	 ...
	 * consider sort avoidance. Presumes a new level has been initialized by
	 * {@link #pushQueryExpressionSuffix()}.
	 *
	 * @param orderByList
	 *            The order by list
	 */
	void pushOrderByList(OrderByList orderByList) {
		if (SanityManager.DEBUG) {
			SanityManager
					.THROWASSERT("pushOrderByList() not expected to be called for "
							+ getClass().getName());
		}
	}
	...
	void pushOffsetFetchFirst(ValueNode offset, ValueNode fetchFirst,
		...
		if (SanityManager.DEBUG) {
			SanityManager
					.THROWASSERT("pushOffsetFetchFirst() not expected to be called for "
							+ getClass().getName());
		}
	}
	...
	void generateResultSet(ExpressionClassBuilder acb, MethodBuilder mb)
		...
		if (SanityManager.DEBUG) {
			SanityManager
					.THROWASSERT("generateResultSet() not expected to be called for "
							+ getClass().getName());
		}
	}
	...
	/**
	 * Replace any DEFAULTs with the associated tree for the default if allowed,
	 * or flag (when inside top level set operator nodes). Subqueries are
	 * checked for illegal DEFAULTs elsewhere.
	 *
	 * @param ttd
	 *            The TableDescriptor for the target table.
	 * @param tcl
	...
	void replaceOrForbidDefaults(TableDescriptor ttd, ResultColumnList tcl,
		...
		if (SanityManager.DEBUG) {
			SanityManager
					.THROWASSERT("replaceOrForbidDefaults() not expected to be called for "
	...
	void markForDistinctScan() {
		if (SanityManager.DEBUG) {
			SanityManager
					.THROWASSERT("markForDistinctScan() not expected to be called for "
							+ getClass().getName());
		}
	}

	/**
	 * Notify the underlying result set tree that the optimizer has chosen to
	 * "eliminate" a sort. Sort elimination can happen as part of preprocessing
	 * (see esp. SelectNode.preprocess(...)) or it can happen if the optimizer
	 * chooses an access path that inherently returns the rows in the correct
	 * order (also known as a "sort avoidance" plan). In either case we drop the
	 * sort and rely on the underlying result set tree to return its rows in the
	 * correct order.
	 *
	 ...
	 * though, is the case of an IndexRowToBaseRowNode, for which we have to
	 * disable bulk fetching on the underlying base table. Otherwise the index
	 * scan could return rows out of order if the base table is updated while
	 * the scan is "in progress" (i.e. while the result set is open).
	 *
	 * In order to account for this (and potentially other, similar issues in
	 * the future) this method exists to notify the result set node that it is
	 * expected to return rows in the correct order. The result set can then
	 * take necessary action to satsify this requirement--such as disabling bulk
	 * fetch in the case of IndexRowToBaseRowNode.
	 *
	 * All of that said, any ResultSetNodes for which we could potentially
	 * eliminate sorts should override this method accordingly. So we don't ever
	 * expect to get here.
	 */
	void adjustForSortElimination() {
		if (SanityManager.DEBUG) {
			SanityManager
					.THROWASSERT("adjustForSortElimination() not expected to be called for "
							+ getClass().getName());
		}
	}

	/**
	 * Same goal as adjustForSortElimination above, but this version takes a
	 * RequiredRowOrdering to allow nodes to adjust based on the ORDER BY
	 * clause, if needed.
	 */
	void adjustForSortElimination(RequiredRowOrdering rowOrdering)
			throws StandardException {
		/*
		 * Default is to ignore the row ordering; subclasses must override if
		 * they need to use it.
		 */
	...
	/**
	 * Count the number of distinct aggregates in the list. By 'distinct' we
	 * mean aggregates of the form:
	 * <UL>
	 * <I>SELECT MAX(DISTINCT x) FROM T<\I><\UL>
	 *
	 ...
	 * {@link #push} before adding the clauses via {@link #setOffset},
	 * {@link #setFetchFirst}, {@link #setOrderByList} and
	 * {@link #setHasJDBCLimitClause}.
	 */
	static class QueryExpressionClauses {
		...
		void push() {
			...
			if (s > 0 && obl.get(s - 1) == null && offset.get(s - 1) == null
	...
	/**
	 * Set up a new level for order by and fetch/offset clauses. See Javadoc for
	 * {@link ResultSetNode.QueryExpressionClauses}. Overridden by implementors
	 * of pushOrderByNode, pushOffsetFetchFirst.
	 */
	...
	void printQueryExpressionSuffixClauses(int depth, QueryExpressionClauses qec) {
		...
		for (int i = 0; i < qec.size(); i++) {
			...
			if (hasJDBCLimitClause != null) {
				printLabel(depth, "hasJDBCLimitClause:" + hasJDBCLimitClause
						+ "\n");
			}
		}
	}
